...
class TestGenerateEmbers:
    ...
    def test_mixed_sources_ranked_by_score(self) -> None:
        ...
        result = generate_embers([mem], ["feel_safe"], [_notion()])
        assert len(result) == 2
    ...
    def test_questions_are_not_an_ember_source(self) -> None:
        result = generate_embers([], [], [])
        assert all("still wondering" not in ember.lower() for ember in result)
